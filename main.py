...
def run(args):
    ...
    tr_transformations, ts_transformations = get_tfs()
    tr_dl, val_dl, ts_dl, classes = get_dls(root = args.root, tr_transformations = tr_transformations, ts_transformations = ts_transformations, bs = args.batch_size)
    ...
    if args.train_framework == "pl":
        ...
        trainer = pl.Trainer(max_epochs = args.epochs, accelerator="gpu", devices = args.devices, strategy = "ddp", 
                             logger = wandb_logger, 
                             # fast_dev_run = True,
                             callbacks = [EarlyStopping(monitor = "val_loss", mode = "min", patience = 5), ImagePredictionLogger(val_samples = val_samples, cls_names = classes),
        ...
        # Test the model
        # this_checkpoint = sorted(glob(f"{args.save_model_path}/*"), key = os.path.getmtime)[-1]
        # print(f"Conducting inference using on the test dataloader using the trained model at {this_checkpoint}")
        # trainer.test(ckpt_path = this_checkpoint, dataloaders = ts_dl)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--root", type = str, default = "/mnt/data/dataset/bekhzod/im_class/fruits", help = "Path to data")
