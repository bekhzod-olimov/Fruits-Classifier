...
def run(args):
    ...
    state_dict = f"{args.save_model_path}/{args.dataset_name}_best_model_{args.train_framework}.pth" if args.train_framework == "py" else sorted(glob(f"{args.save_model_path}/*"), key = os.path.getmtime)[-1]
    model.load_state_dict(torch.load(state_dict, map_location = "cpu"), strict = True) if args.train_framework == "py" else model.load_state_dict(get_state_dict(state_dict), strict = True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-dn", "--dataset_name", type = str, default = "fruits", help = "Dataset name for training")
