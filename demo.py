...
def run(args):
    ...
    tr_tfs, ts_tfs = get_tfs()
    
    # Set a default path to the image
    default_path = "sample_ims/durian.jpg"
    ...
    im, result = predict(m = m, path = file, tfs = ts_tfs, cls_names = cls_names) if file else predict(m = m, path = default_path, tfs = ts_tfs, cls_names = cls_names)
